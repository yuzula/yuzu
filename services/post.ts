...
export const get = retryDecorator(async (id: number) => {
...
export const getAll = retryDecorator(
  async ({
    communityDomainName,
    sortBy = 'hot',
    filterBy = 'all'
...
export const create = retryDecorator(
  async ({ communityDomainName, userId, content, isPrivate }: CreateParams) => {
    const response = await supabase
      .from('posts')
      .insert({
...
export const getVote = retryDecorator(
  async ({ postId, userId }: GetPostVotesParams) => {
    const response = await supabase
      .from('post_votes')
      .select()
      .eq('post_id', postId)
...
export const registerVote = retryDecorator(
  async ({ postId, userId, vote }: VoteParams) => {
